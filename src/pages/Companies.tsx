...
import { ArrowRight, Search, Filter, X, Briefcase, Globe, LayoutGrid, List } from 'lucide-react';
...
export function Companies() {
  ...
  const services = useMemo(() => {
    const allServices = companies.flatMap(c => c.services);
  ...
  return (
    <div className="bg-white min-h-screen font-sans selection:bg-cyan-200 selection:text-slate-900">
      ...
      <PageHeader
        highlight="Hub des Filiales"
        title={<>Le Portefeuille <br/><span className="text-[#90A1B9] bg-clip-text">ITM Holding</span></>}
        subtitle="Explorez notre écosystème de 15 entreprises interconnectées, conçues pour offrir des solutions intégrées à l'échelle continentale."
        backgroundImage={HERO_IMAGE}
      />

      {/* 2. FILTER & SEARCH BAR - Sticky & Premium */}
      <section className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm transition-all">
         <div className="w-full px-8 md:px-32 lg:px-[15vw] py-6">
            <div className="flex flex-col lg:flex-row gap-6 justify-between items-center">
               ...
               <div className="relative w-full lg:max-w-md group">
                  ...
                  <Input 
                    placeholder="Rechercher une filiale, un service..." 
            ...
            {(selectedIndustry !== "all" || selectedService !== "all" || searchQuery) && (
               <div className="flex items-center gap-2 mt-4 text-sm animate-in fade-in slide-in-from-top-2">
                  <span className="text-slate-400 mr-2">Filtres actifs :</span>
                  ...
                  <Button variant="link" className="text-slate-500 hover:text-slate-900 text-xs h-auto p-0 ml-4" onClick={clearFilters}>
                     Tout effacer
                  </Button>
               </div>
            )}
            ...
            <div className="flex flex-col md:flex-row justify-between items-end mb-20 border-b border-slate-200 pb-10">
               <div>
                  <div className="flex items-center gap-3 mb-6">
                     ...
                     <span className="text-cyan-600 font-bold tracking-widest uppercase text-xs">Portfolio
                     </span>
                  </div>
                  <h2 className="text-4xl md:text-6xl font-light text-slate-900 leading-tight">
                     Nos pôles <br/>
                     <span className="font-bold">d'excellence.</span>
                  </h2>
               </div>
               ...
               <div className="flex flex-col items-end mt-8 md:mt-0">
                  ...
                  <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-2">
                     {filteredCompanies.length > 1 ? 'Entités Actives' : 'Entité Active'}
                  </p>
               </div>
            </div>
            ...
            <AnimatePresence mode="popLayout">
               <div className={viewMode === "grid" 
                  ...
                  {filteredCompanies.map((company, index) => {
                     // Background images for cards (cycling)
                     const bgImages = [
                     ...
                     if (viewMode === 'grid') {
                        return (
                           <motion.div
                              ...
                              className="w-full aspect-[3/4] relative group overflow-hidden bg-slate-900 cursor-pointer border border-slate-800 shadow-xl"
                           >
                           ...
                           <Link to={`/companies/${company.id}`} className="block h-full">
                              <div className="group bg-white border border-slate-200 h-full transition-all duration-500 hover:shadow-xl hover:shadow-slate-200/50 hover:-translate-y-1 flex flex-col md:flex-row gap-6 p-6 items-center">
                                 ...
                                 <div className="border-t border-slate-100 pt-6 flex items-center justify-between md:w-auto md:border-t-0 md:pt-0 md:pl-6 md:border-l">
                                    <span className="text-sm font-bold text-slate-900 group-hover:text-cyan-600 transition-colors mr-4">Découvrir</span>
               ...
               {filteredCompanies.length === 0 && (
                  <div className="text-center py-24">
                     ...
                     <Button variant="link" onClick={clearFilters} className="mt-4 text-cyan-600">
